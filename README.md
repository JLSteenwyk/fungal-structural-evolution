...
Status: proteomes and broad-marker QC are complete for **501 fungal entries and 25 genome-backed outgroups (526 entries)**. The [taxon-label audit](metadata/taxon_label_review.tsv) flags 21 incompletely identified fungal entries; [assembly-linked literature review](docs/taxon-identity-sensitivities.md) identifies two curated hybrids. These entries are not yet established as 501 unique fungal species. All 125 profile and MAFFT marker alignments and their coding-sequence projections are complete. The profile guide tree is complete, and the first supported C20-PMSF species-tree analysis is running. The MAFFT guide, supported marker trees, full orthology assignment and whole-proteome domain searches remain running. Supported species trees, reconciled orthology and the full evolutionary analyses remain incomplete. Full-scale design; no separate pilot.

Confidence-qualified paired-marker coverage now reaches **525 entries (500 fungal entries and all 25 outgroups)** across three separately prepared AlphaFold and ESMFold cohorts. The follow-on 4,252-model ESMFold cohort supplies 88 ready markers, 3,986 usable taxon–marker combinations and 67 previously unrepresented entries. The descriptive union contains 22,220 usable combinations; *Amoeboaphelidium protococcarum* is the remaining uncovered entry. These counts do not establish 500 unique species, uniform coverage or a combined phylogenetic analysis. Same-method cohort integration and prediction-source controls remain necessary. [Coverage evidence and limitations](docs/structure-prediction-workflow.md#follow-on-paired-inputs-completed-and-coverage-expanded).
