...
September 22 availability refresh: the current downloaded AlphaFold catalog plus completed ESMFold batches cover **56,171/59,840 recovered marker/protein records (93.9%)**, leaving 3,669 without models in these catalogs. Full frozen-log selection readback and independent marker/taxon joins passed. This supersedes the older 65.1% availability checkpoint; it is neither proteome-wide nor confidence-qualified coverage. Newly cataloged AlphaFold models await downstream integration. [Current coverage and evidence](docs/completed-prediction-inventory-20260922.md#current-combined-availability-939-of-recovered-marker-records).

September 22 prediction update: the five completed ESMFold cohorts contain **25,322 distinct protein sequences**, including the complete 10,522-model original queue and the 5,510 long and 4,363 extended-length models. The combined inventory and independent full source-record readback passed, preserving seven prediction configurations. Exact matches yield 25,509 marker/protein links across 295 taxa and 124 markers; this is local model availability before confidence qualification, separate from AlphaFold coverage. Residue mappings passed for the original, long and extended cohorts; the combined mapping has also passed independent residue-level readback, and original-cohort feature/confidence processing has passed full directional PAE readback. Long and extended cohorts remain in processing. These additions are not yet integrated into the evolutionary results below and do not represent proteome-wide structural coverage. [Inventory and remaining integration](docs/completed-prediction-inventory-20260922.md).

Status: proteomes and broad-marker QC are complete for **501 fungal entries and 25 genome-backed outgroups (526 entries)**. The [taxon-label audit](metadata/taxon_label_review.tsv) flags 21 incompletely identified fungal entries; [assembly-linked literature review](docs/taxon-identity-sensitivities.md) identifies two curated hybrids. These entries are not yet established as 501 unique fungal species. All 125 profile and MAFFT marker alignments and their coding-sequence projections are complete. Both full-taxon profile and MAFFT guide trees are complete and audited. Two of four supported C20-PMSF species-tree executions passed profile, tree and empirical bootstrap readback; their same-alignment comparison shares 516/523 internal splits, with no incompatible split pair strongly supported in both runs. The remaining crossed analyses and biological validation are incomplete. The final retained-family tree repair is complete and independently validated. All 64 additional full-proteome Pfam chunks and marker annotations are joined in a database with independently verified hit fields and all 5,815,847 protein links. Four-policy clan competition passed complete independent readback; candidate architecture construction is complete with repeats, annotation types and overlap uncertainty retained; independent architecture readback passed. Both complete family partitions are now linked to candidate annotations and pass independent full bridge readback; within-family annotation variation and independent reconstruction of all 5,266,820 family/policy rows are complete; exact-membership guide comparisons are complete: 656,507 shared families contain 99.35% of proteins and have identical architecture metrics under each policy. Biological architecture resolution remains pending. Clade-specific discovery across all 60 distinct guide-defined clades and independent merged-partition readback are complete: each guide partition contains all 5,815,847 proteins exactly once. All 63,949 distinct new-family alignments and trees are complete and passed independent native-parser readback; 32,637 retained trees are also validated and mapped to both expanded partitions. Complete expanded reconciliation inputs now pass independent native readback for all 95,484 profile-partition and 95,594 MAFFT-partition trees, including the repaired final family. A CPU-only controller has launched to stage isolated copies and execute reconciliation sequentially under both homogeneous guide alternatives; final output validation remains pending. Synthetic output checks found [small-family ortholog-table omissions](docs/native-small-family-output-omissions-20260922.md); a separately tested missing-pair supplement is queued after both native runs; full output validation remains required. All 125/125 marker gene trees now pass full input and support-split readback; [complete marker diagnostics](docs/complete-marker-diagnostics-20260922.md) retain supported guide conflicts and unresolved rooting. Supported species trees, reconciled orthology and the full evolutionary analyses remain incomplete. Full-scale design; no separate pilot.

Confidence-qualified paired-marker coverage reaches **525 entries (500 fungal entries and all 25 outgroups)** across separate AlphaFold and ESMFold datasets, with 22,677 usable taxon–marker combinations in their descriptive union. The three audited ESMFold acquisition cohorts are now integrated: 10,048 models yield 89 ready markers across 292 taxa. Paired branch fits and direct coordinate comparisons are audited; expanded ESMFold site-rate optimization, 89-marker conditional coupling and an 88-marker copy-review omission sensitivity are complete, including marker resampling. AlphaFold optimization, full124/44,719-site coupling and omission123/44,410-site sensitivity are also complete and checked against leave-one-marker-out estimates. *Amoeboaphelidium protococcarum* remains uncovered. These counts do not establish 500 unique species, uniform coverage or pooled evolutionary inference. Prediction-source controls remain necessary. [Coverage records](metadata/combined_predictor_coverage_union.json).
