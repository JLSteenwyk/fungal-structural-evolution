...
- `expanded_marker_model_catalog_receipt.json`: independently hash-verified GDM coordinate catalog for PAE prefetch, tied to the expanded mapper's frozen inventory; not a residue-mapping completion receipt.
- `expanded_marker_coverage_receipt.json`, `expanded_marker_taxon_coverage.tsv`, and `expanded_marker_lineage_coverage.tsv`: full 526-taxon × 125-marker availability audit. Counts distinguish catalog models, reuse candidates outside the catalog, no candidate in the completed query snapshot, and unrecovered sequences. Lineage fractions use all marker slots; per-taxon recovered-marker fractions have a separate denominator. These are availability states, not inferred absences or confidence-qualified observations.
- `expanded_pae_resource_plan.json`: prelaunch resource envelope and catalog-based launch gate for expanded confidence acquisition; final downstream binding requires agreement with the completed residue mapping.

- `busco_gene_copy_audit_receipt.json` and `busco_gene_copy_summary.tsv`: full 526-taxon raw-BUSCO linkage to explicit gene/product annotations. `busco_gene_copy_review.tsv` is the subset of duplicated calls or original hits excluded by representative selection; missing source hits are not recalled as missing BUSCOs. Distinct resolved gene counts exclude unresolved mappings and are not biological duplication-event counts.
- `expanded_marker_mapping_receipt.json`, `expanded_marker_mapping_coverage.tsv`, and `expanded_marker_catalog_mapping_agreement.json`: completed expanded residue mapping and exact semantic agreement with the prefetch model catalog. These do not establish native-feature/PAE audit completion.
