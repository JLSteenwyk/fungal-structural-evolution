#!/usr/bin/env python3
"""Freeze a longer-marker queue and explicitly extrapolated resource scenarios."""
import argparse
import hashlib
import json
import math
...
def main():
    ...
    p.add_argument('--minimum-length', type=int, default=513)
    p.add_argument('--maximum-length', type=int, default=768)
    a = p.parse_args()
    if a.output.exists():
        raise FileExistsError('Use a new immutable queue')
    if not 513 <= a.minimum_length <= a.maximum_length <= 768:
        raise ValueError('This resource extrapolation is restricted to lengths 513–768')
    ...
    if len(observations) < 100:
        raise ValueError('Insufficient upper-short-length timing observations')
    ...
    plan = {'status': 'extrapolated_planning_scenarios_not_measured_long_protein_performance',
            'observed_receipts': len(observations), 'observed_length_range': [385, 512],
            ...
            'limitations': 'Quadratic/cubic timing and quadratic total-memory scaling are explicit scenarios, not validated complexity bounds or runtime/confidence intervals. Runtime, allocator overhead and fragmentation can differ beyond 512 residues. Timing receipts are observations, not an independent structure-quality audit. No extrapolation above 768 residues or to the full proteome atlas.'}
