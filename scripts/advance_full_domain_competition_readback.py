#!/usr/bin/env python3
"""Wait for the exact competition producer, then require and audit its complete outputs."""
import argparse
import hashlib
import json
...
def main():
    ...
    state('waiting_for_exact_competition_producer')
    while True:
        try:
    ...
    try:
        ...
        for path,digest in config['pins'].items():
            assert sha(path)==digest,path
        state('running_independent_competition_readback')
        command=['/home/bizon/anaconda3/bin/python','scripts/readback_full_domain_competition.py','--plan',config['plan'],'--output',str(Path(plan['output'])/'readback.json')]
        with (output/'readback.log').open('w') as log:
            subprocess.run(command,stdout=log,stderr=subprocess.STDOUT,check=True)
        result=json.loads((Path(plan['output'])/'readback.json').read_text())
        assert result['status']=='passed_complete_independent_domain_competition_readback'
        state('complete_independent_competition_readback',readback_sha256=sha(Path(plan['output'])/'readback.json'))
