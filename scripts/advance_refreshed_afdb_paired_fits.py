#!/usr/bin/env python3
"""Run all eligible refreshed AlphaFold paired fits after input readback."""
import argparse
import json
import os
...
def main():
    ...
    if (previous['status']!='complete_refreshed_afdb_paired_inputs_and_independent_array_readback'
    ...
    verify()
    result=dict(status='complete_refreshed_afdb_supported_paired_point_fits_and_report_audit',markers=len(ready),fits=ar['fits'],
