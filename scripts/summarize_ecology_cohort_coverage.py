...
def main():
    ...
    p.add_argument('--source',action='append',required=True,help='Unique cohort label=paired-input directory')
    p.add_argument('--output',type=Path,required=True)
    a=p.parse_args()
    sources=[x.split('=',1) for x in a.source]
    if any(len(x)!=2 or not x[0] or not x[1] for x in sources) or len({x[0] for x in sources})!=len(sources):raise ValueError('Unique named cohort paths required')
    ...
    for source,folder in sources:
        b=Path(folder);r=json.loads((b/'receipt.json').read_text())
    ...
    rows=[{'taxon_id':r['taxon_id'],'species_name':r['species_name'],'source_classification':r['state'],'provisional_transition_group':r['provisional_transition_group'],
           **{source+'_usable_markers':len(totals[source][r['taxon_id']]) for source,folder in sources},
    ...
    table=a.output/'taxon_coverage.tsv';contrast=a.output/'contrast_coverage.tsv';write(table,rows);write(contrast,contrasts)
    result={'status':'complete_ecology_paired_coverage_join','ecology_evidence_sha256':sha(ecological),'host_evidence_sha256':sha(hosts),'paired_input_paths':dict(sources),'paired_input_receipt_sha256':source_receipts,'script_sha256':sha(Path(__file__)),
            'ecologically_curated_species':len(rows),'with_any_source_usable_marker':sum(any(r[source+'_usable_markers']>0 for source,folder in sources) for r in rows),'table_sha256':sha(table),'contrast_table_sha256':sha(contrast),
