#!/usr/bin/env python3
"""Prepare and independently read back expanded paired inputs after qualification."""
import argparse
import csv
import json
import os
...
def main():
    ...
    baseline_audit=json.loads((ROOT/plan['baseline_readback']).read_text())
    if (baseline_audit['status']!='passed_complete_paired_inputs_from_qualified_arrays_readback'
            or baseline_audit['source_receipt_sha256']!=sha(ROOT/plan['baseline']/'receipt.json')):
        raise ValueError('Verified baseline array readback required')
    encoding=checked_receipt(ROOT/plan['encodings'])
    if encoding['models']!=25322 or encoding['plan_sha256']!=sha(ROOT/plan['encoding_union_plan']):
        raise ValueError('Wrong qualified encoding union')
    ...
    verify()
    receipt=dict(status='complete_full_cohort_paired_inputs_and_independent_array_readback',plan_sha256=plan_sha,
                 ...
                 scope='Complete expanded model cohort with unchanged confidence and eligibility rules; all paired characters independently reconstructed. Coverage changes are not evolutionary effects. Tree fitting, branch uncertainty, direct-geometry benchmarking and copy/annotation sensitivities remain required.')
