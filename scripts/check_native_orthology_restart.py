...
def main():
    ...
    ap.add_argument('--wrapper', type=Path, help='Optional explicitly traced native launcher')
    ap.add_argument('--stall-timeout-seconds', type=float, default=21600)
    a = ap.parse_args()
    ...
    for case in ['missing_ids_tree', 'prepared_ids_tree']:
        ...
        launcher = ([str(executable.parent/'python'), str(a.wrapper.resolve()),
                     '--stall-timeout-seconds', str(a.stall_timeout_seconds), '--'] if a.wrapper else [str(executable)])
        command = launcher + ['--from-trees', str(source), '-s', str(user_tree),
