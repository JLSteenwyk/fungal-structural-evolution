#!/usr/bin/env python3
"""Compare full-cohort site-rate models after rate and exposure producers complete.
...
def main():
    ...
    def producer_live(producer):
        try:
            ...
            if p.create_time()!=producer['created'] or p.status()==psutil.STATUS_ZOMBIE:
                return False
            if p.cmdline()!=producer['command']:
                raise ValueError('Producer command changed')
            return True
        except psutil.NoSuchProcess:
            return False
    def live():
        states = [producer_live(p) for p in c['producers']]
        return any(states)
    verify()
    if not live():
    ...
    while live():
        print('Waiting for rate/exposure producers', [p['pid'] for p in c['producers']],flush=True);time.sleep(30)
    ...
    verify()
    result={'status':'complete_full_cohort_rate_comparison_and_exposure_frame','config_sha256':digest,'prerequisite_receipts':prerequisites,'stages':completed,'interpretation':'All-fit optimization diagnostics, matched rate-model comparison, readback and exposure frame complete. Coupling inference and branch-specific evolutionary conclusions remain separate stages.'}
