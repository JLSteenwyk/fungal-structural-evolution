...
def main():
    ...
    try:
        ...
        if hr['status']!='complete_full_cohort_rate_comparison_and_exposure_frame' or hr['config_sha256']!=c['producer_config_sha256']:raise ValueError('Upstream controller incomplete or mismatched')
        ...
        plan.update(sites=c['sites'],markers=c['markers'],source_frame_sha256=sha(frame/'receipt.json'),source_accessibility_sha256=sha(access/'receipt.json'),memory_allowance_gb=16,output_allowance_gb=4,planning_hours=c['fit_planning_hours'],uncertainty=f"CR1 marker-cluster covariance, finite-sample correction and t({c['markers']-1}) reference; conditional on estimated rates and fixed gene trees. BH across 72 focal tests.",interpretation=c['interpretation'])
        pp=root/'fit_plan.json';pp.write_text(json.dumps(plan,indent=2)+'\n')
        run('coupling',['scripts/fit_conditional_site_coupling.py','--frame',str(frame),'--accessibility',str(access),'--plan',str(pp),'--output',c['fit_output']])
        fits=Path(c['fit_output']);fit=checked(fits)
        if fit['status']!='complete_exploratory_conditional_site_coupling_fits' or fit['fits']!=24 or fit['markers']!=c['markers'] or fit['sites']!=c['sites']:raise ValueError('Incomplete coupling fit grid')
        plan=json.loads(Path(c['resampling_template']).read_text())
        plan.update(source_fit_receipt_sha256=sha(fits/'receipt.json'),markers=c['markers'],leave_one_marker_out_fits=24*c['markers'],memory_allowance_gb=16,output_allowance_gb=4,planning_hours=c['resampling_planning_hours'],method=f"Within-marker centering; paired multinomial resampling of {c['markers']} whole markers, 2000 draws across 24 specifications, and omission of every marker. Expanded-row verification retained.")
        ...
        if rs['status']!='complete_conditional_marker_resampling' or rs['models']!=24 or rs['markers']!=c['markers'] or rs['bootstrap_fits']!=48000 or rs['leave_one_marker_out_fits']!=24*c['markers']:
