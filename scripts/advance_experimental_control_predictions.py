#!/usr/bin/env python3
"""Start the frozen experimental_control queue after a successful predecessor frees its GPU."""
...
def main():
    ...
    launch={'status':'launching_experimental_control_predictions','controller_config_sha256':config_sha,'predecessor_chunk_sha256':sha(chunk),'gpu_process_observation':apps,'command':command,'gpu_uuid':c['gpu_uuid'],'unix_time':time.time()}
    (control/'launch.json').write_text(json.dumps(launch,indent=2)+'\n');print('Launching experimental_control predictions',flush=True)
    ...
    receipt={'status':'complete_experimental_control_prediction_chunk' if success else 'experimental_control_prediction_requires_review','exit_code':result.returncode,'controller_config_sha256':config_sha,'launch_sha256':sha(control/'launch.json'),'prediction_chunk_sha256':sha(final) if final.exists() else None,'interpretation':'Controller completion verifies clean chunk termination, not an independent model/PAE artifact audit.'}
