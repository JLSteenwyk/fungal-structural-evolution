#!/usr/bin/env python3
"""Recover reconciliation using verified completed guides and fresh incomplete-guide copies."""
import argparse
import csv
import json
import os
...
def validate_completed_stage(record, guide, expected_original_plan):
    stage = record['stage']
    if (record['execution_plan_sha256'] != expected_original_plan
            or stage['guide'] != guide
            or stage['status'] != 'native_execution_complete_pending_full_output_readback'
            or stage['source_inputs_unchanged'] is not True
            or not stage['mandatory_artifacts']):
        raise ValueError('Completed guide provenance/status differs')
    return stage
...
def main():
    ...
    try:
        for guide in receipt['guides']:
            verify()
            name = guide['guide']
            if name in plan.get('reuse_completed', {}):
                reuse = plan['reuse_completed'][name]
                record = read(ROOT / reuse['stage_receipt'])
                stage = validate_completed_stage(record, name, reuse['original_plan_sha256'])
                original_result = ROOT / stage['result']
                original_base = original_result.parent
                manifest = inputs / name / 'copied_files.tsv'
                if sha(manifest) != guide['manifest_sha256']:
                    raise ValueError('Changed reused-guide manifest')
                with manifest.open() as handle:
                    for row in csv.DictReader(handle, delimiter='\t'):
                        if sha(original_base / row['relative_path']) != row['sha256']:
                            raise ValueError('Completed guide source differs from frozen input')
                for relative, digest in stage['mandatory_artifacts'].items():
                    if sha(original_result / relative) != digest:
                        raise ValueError('Changed reused native artifact')
                (output / name).symlink_to(original_base.resolve(), target_is_directory=True)
                reused = dict(stage, reused_stage_receipt=reuse['stage_receipt'],
                              reused_stage_receipt_sha256=sha(ROOT / reuse['stage_receipt']))
                state['stages'].append(reused)
                state.update(status='reused_verified_completed_guide', guide=name)
                save()
                continue
        ...
        for stage in state['stages']:
            for relative, digest in stage['mandatory_artifacts'].items():
                if sha(ROOT / stage['result'] / relative) != digest:
                    raise ValueError('Completed output changed before combined handoff')
