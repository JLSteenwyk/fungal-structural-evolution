#!/usr/bin/env python3
"""Gate full-atlas CPU clustering on the completed structural database."""
import argparse
...
def main():
    ...
    if receipt['status']!='complete_whole_proteome_foldseek_database_and_sequence_readback' or receipt['plan_sha256']!=sha(plan['database_plan']):raise ValueError('Database verification incomplete')
    for name,h in receipt['artifacts'].items():
        if sha(db/name)!=h:raise ValueError('Database artifact changed')
    expected=set()
    with (db/'structures.lookup').open() as f:
        for line in f:
    ...
    prefix=str(db/'structures');cluster=str(out/'clusters');commands=[
