#!/usr/bin/env python3
"""Audit revised rate exports, including exact topology preservation of reviewed branch-length starts."""
...
def main():
    ...
    for m in ready:
        ...
        for label in labels:
            ...
            if sha(rp)!=r['fit_receipts'][marker+'/'+label] or fr['parent_config_sha256']!=r['config_sha256'] or request['parent_config_sha256']!=r['config_sha256']:raise ValueError('Fit provenance differs')
            start_tree=Path(request['command'][request['command'].index('-te')+1])
            if request['topology_sha256']!=sha(start_tree) or set(tree_edges(start_tree,taxa))!=set(tree_edges(topology,taxa)):raise ValueError('Starting tree identity/topology differs')
            if request['topology_sha256']!=sha(topology):
                revision=c.get('revision',{})
                if (revision.get('marker')!=marker or revision.get('fit')!=label or request.get('reference_topology_sha256')!=sha(topology) or request.get('reviewed_refit_receipt_sha256')!=revision.get('refit_receipt_sha256')):raise ValueError('Unreviewed alternate branch-length start')
