...
def main():
    ...
    for stage in plan['stages']:
        ...
        if stage['name'] == 'readback':
            # This audit writes a self-contained receipt, not an artifact manifest.
            result = json.loads(receipt.read_text())
            arguments = stage['arguments']
            for key in ('comparisons', 'inputs', 'snapshot', 'pae'):
                source = Path(arguments[arguments.index('--' + key) + 1])
                checked_receipt(source)
                assert result['source_receipts'][key] == sha(source / 'receipt.json'), key
            assert result['script_sha256'] == sha(arguments[0])
            assert sum(result['counts'].values()) == plan['pairs']
            assert result['independent_geometry_checks'] == plan['markers']
            assert len({r['marker'] for r in result['checked_pairs']}) == plan['markers']
        else:
