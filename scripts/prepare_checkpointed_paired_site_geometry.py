...
def main():
    ...
    for name in ['inputs', 'snapshot', 'pae', 'output', 'checkpoints']:
    ...
    lock = (args.checkpoints / '.lock').open('a')
    fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
    config = {'sources': {n: sha(getattr(args, n) / 'receipt.json') for n in ['inputs', 'snapshot', 'pae']},
              'scripts': {n: sha(Path(__file__).with_name(n)) for n in
                          [Path(__file__).name, 'assess_pae_sensitivity.py', 'compare_marker_structures.py',
                           'prepare_paired_phylogenetic_inputs.py', 'retrieve_marker_pae.py']},
              'markers': [r['marker'] for r in ready]}
    ...
    config_path = args.checkpoints / 'config.json'
    if config_path.exists():
        if json.loads(config_path.read_text()) != config: raise ValueError('Checkpoint source/config changed')
    else:
        if list(args.checkpoints.glob('*.json')): raise ValueError('Checkpoint config missing')
    ...
    for fitted_marker in ready:
        ...
        checkpoint = args.checkpoints / (marker + '.json')
        if checkpoint.exists():
            saved = json.loads(checkpoint.read_text())
            if saved['config_sha256'] != config_hash or saved['marker'] != marker:
                raise ValueError('Checkpoint identity changed')
            payload = json.dumps([saved['results'], saved['exclusions']], sort_keys=True, allow_nan=False)
            if hashlib.sha256(payload.encode()).hexdigest() != saved['payload_sha256']:
                raise ValueError('Checkpoint payload changed')
            expected = int(fitted_marker['eligible_taxa'])
            if len(saved['results']) + len(saved['exclusions']) != expected * (expected-1) // 2:
                raise ValueError('Checkpoint pair grid incomplete')
            results.extend(saved['results']); exclusions.extend(saved['exclusions'])
            print(marker, 'restored checkpoint', flush=True)
            continue
        result_start, exclusion_start = len(results), len(exclusions)
        ...
        for a, b in combinations(sorted(aa), 2):
            ...
            results.append(row)
        new_results, new_exclusions = results[result_start:], exclusions[exclusion_start:]
        payload = json.dumps([new_results, new_exclusions], sort_keys=True, allow_nan=False)
        atomic_json(checkpoint, dict(marker=marker, config_sha256=config_hash,
                    payload_sha256=hashlib.sha256(payload.encode()).hexdigest(),
                    results=new_results, exclusions=new_exclusions))
        print(marker, 'benchmarked and checkpointed', flush=True)
    ...
    receipt = {'status': 'complete_paired_site_geometry', 'markers': len(ready),
        ...
        'checkpoint_config_sha256': config_hash, 'checkpoint_directory': str(args.checkpoints),
