...
def main():
    ...
    pending=[r for r in read_table(args.inputs/'queue_disposition.tsv') if r['queue_disposition']=='short_prediction_eligible']
    summaries=[]
    ...
    result={'status':'complete_followon_short_queue_resource_projection','pending_short_predictions':len(pending),'observed_predictions':len(measurements),
        ...
        'planned_gpu':'Same authorized GPU 1 after existing run finishes; recheck availability before launch',
        'planning_vram_gb':24,'planning_output_gb':20,'cost':'Existing host; no paid resources',
        'input_receipt_sha256':sha(args.inputs/'receipt.json'),'prediction_config_sha256':config_sha,'script_sha256':sha(Path(__file__)),
