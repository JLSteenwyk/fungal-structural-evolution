#!/usr/bin/env python3
"""Full bridge readback with explicit family-index lookup to avoid guide scans."""
import argparse
import csv
import hashlib
import itertools
...
def audit(plan_path):
    ...
    state('architecture_link_readback')
    ...
    for guide in source['guides']:
        families = proteins = 0
        state('family_readback', guide=guide['name'], families=0, proteins=0)
        with open(guide['crosswalk']) as f:
            for entry,row in itertools.zip_longest(partitions(guide['clusters']),csv.DictReader(f,delimiter='\t')):
                ...
                name, genes = entry
                actual = [r[0] for r in db.execute('SELECT native_gene_id FROM assignments INDEXED BY assignments_family WHERE guide=? AND family=? ORDER BY native_gene_id',(guide['name'],name))]
                if actual != genes:
