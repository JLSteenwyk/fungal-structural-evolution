...
def main():
    ...
    p.add_argument('--title',default='Prediction–experiment agreement: available reference coverage')
    a=p.parse_args()
    ...
    reference_models={r['predicted_model_id'] for r in rows+excluded}
    ...
    fig,(left,right)=plt.subplots(1,2,figsize=(12,max(10,.24*len(baseline)+2)),gridspec_kw={'width_ratios':[1.4,1]})
    ...
    fig.text(.5,.04,f'n = accepted chain/model comparisons at pLDDT ≥0; points are dependent descriptive summaries.\nThresholds change residue subsets and protein coverage. {len(reference_models)-len(baseline)} reference proteins fail coverage at every threshold.\nExperimental quality, biological context and training independence remain unresolved.',ha='center',fontsize=9)
    fig.tight_layout(rect=[0,.10,1,.96])
    for ext in ['svg','png','pdf']:fig.savefig(a.output/('experimental_agreement.'+ext),dpi=180)
    plt.close(fig)
    receipt={'status':'complete_experimental_agreement_figure','comparison_receipt_sha256':sha(a.comparisons/'receipt.json'),'summary_receipt_sha256':sha(a.summary/'receipt.json'),'script_sha256':sha(Path(__file__)),'title':a.title,'reference_proteins':len(reference_models),'plotted_proteins':len(baseline),'recomputed_threshold_summary':table,'interpretation':'Every plotted per-protein count and median independently recomputed from accepted comparison rows. Descriptive pinned reference dataset; no inferential independence or accuracy claim; no point is drawn for absent threshold coverage.','artifacts':{f.name:sha(f) for f in a.output.iterdir()}}
