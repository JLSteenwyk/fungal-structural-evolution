#!/usr/bin/env python3
"""Integrate disjoint audited ASA cohorts without changing original entry files."""
import argparse
import json
import os
...
def main():
    ...
    if inventory['status'] != 'complete_disjoint_same_method_inventory_union' or mapping['source_inventory_sha256'] != sha(a.inventory / 'inventory.jsonl'):
    ...
    for argument in a.cohort:
        ...
        for row in rows:
            ...
            if sid in entries or combined.get(sid) != m:
            ...
            if sha(table) != entry['table_sha256'] or sha(ROOT / m['path']) != m['sha256']:
    ...
    result = {'status': 'complete_disjoint_audited_accessibility_union', 'models': len(entries),
              ...
              'mapping_receipt_sha256': sha(a.snapshot / 'receipt.json'), 'source_cohorts': sources,
