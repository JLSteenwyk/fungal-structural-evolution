#!/usr/bin/env python3
"""Summarize and independently read back full-cohort site parsimony and exposure.
...
def main():
    ...
    while live():
        print('Waiting for paired-fit producer',c['producer']['pid'],flush=True);time.sleep(30)
    ...
    verify()
    result={'status':'complete_full_cohort_site_parsimony_exposure_and_readback','config_sha256':digest,'prerequisite_receipts':prerequisites,'stages':completed,'interpretation':'All paired-site minimum character changes and extant exposure summaries read back. These are descriptive lower bounds on fixed AA topologies, not branch rates, ancestral exposure, or coupling tests.'}
