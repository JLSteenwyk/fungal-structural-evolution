#!/usr/bin/env python3
"""Run native structural features and complete confidence readback after residue QC."""
import argparse
import json
import os
...
def main():
    ...
    try:
        ...
        if (predecessor['status'] != 'complete_local_cohort_mapping_and_residue_readback'
                or predecessor['plan_sha256'] != sha(ROOT / plan['mapping_plan'])
                or predecessor['mapping_receipt_sha256'] != sha(mapping_receipt)
                or predecessor['readback_receipt_sha256'] != sha(residue_receipt)
                or residue['status'] != 'passed_full_local_residue_mapping_readback'
        ...
        result = dict(status='complete_native_features_and_original_pae_context_readback', models=plan['models'],
                      ...
                      scope='Full cohort native feature extraction, coordinate reconstruction and directional confidence readback. Biological model accuracy, alignment uncertainty and evolutionary analyses remain separate.')
