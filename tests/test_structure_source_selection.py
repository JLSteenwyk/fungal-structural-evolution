...
from map_marker_structures import source_candidates, inventory_key
...
class InventoryIdentityTests(unittest.TestCase):
    def test_local_identity_requires_no_uniprot_accession(self):
        self.assertEqual(inventory_key({'record_id': 'S' + 'a' * 64}), 'S' + 'a' * 64)
        self.assertEqual(inventory_key({'uniprot_accession': 'P12345'}), 'P12345')

    def test_missing_or_malformed_identity_rejected(self):
        for row in [{}, {'record_id': 4}, {'record_id': ' '}]:
