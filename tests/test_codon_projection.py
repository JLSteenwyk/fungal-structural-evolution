...
class AnnotatedCodonSpan(unittest.TestCase):
    ...
    def test_published_initial_phase_already_removed(self):
        parts=[{'sequence':'s','start':1,'end':9,'strand':'+','phase':'2'}]
        dna,initial,tail,_,representation,raw=project(parts,{'s':'CCATGAAAT'},'ATGAAAT')
        self.assertEqual((dna,initial,tail),('ATGAAA',2,1))
        self.assertEqual(representation,'initial_phase_already_removed')
        self.assertEqual(raw[:initial],'CC')
