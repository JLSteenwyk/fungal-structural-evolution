...
supplements. Full-table protein/species/family identity audits and count-table
readbacks have now passed for all 526 tables in each guide: 839,296,556 profile
rows and 839,250,464 MAFFT rows. Both services exited successfully. Their
software-fixture and corruption checks also pass. Cross-row pair duplication,
...
| Structural atlas | ESMFold inventory: 25,322 models, with full accessibility projection and normalization readbacks complete. Audited marker catalog: 56,171/59,840 recovered marker–taxon entries have models. Refreshed AlphaFold marker confidence and PAE-context readback complete for 30,588 models. Whole-proteome AlphaFold catalog: 1,290,278 unique models partitioned into 400,127 candidate structural clusters; full cluster composition source readback complete | metadata/esmfold_all_completed_inventory_receipt.json; metadata/current_marker_availability_receipt.json; metadata/completed_esmfold_accessibility_controller_receipt_20260923.json; metadata/current_afdb_confidence_completed_readback.json; metadata/whole_proteome_cluster_composition_completed_readback_receipt.json; GPU prediction paused, 3,669 marker entries still uncovered. Domain coordinate audit, full database readback, 70,537-cluster partition and all 594,797 boundary-pair comparisons complete; see metadata/full_domain_clustering_completed_receipt_20260923.json and metadata/domain_boundary_clusters_completed_readback_20260923.json. Domain confidence/parameter sensitivity and phylogenetic integration remain pending. Candidate clusters are not established orthogroups; whole-proteome confidence qualification and atlas completion remain pending |
...
### September 16: full reconciliation workload and large-family concentration measured

Measured every audited family's taxon multiplicities without expanding protein pairs: 5.18 billion unordered candidate cross-species comparisons across 104,138 families. Export readback checked all rows and totals. Flat pair storage would be large; grouped native outputs remain preferable, and these dimensions are not runtime or actual ortholog counts. The inventory exposed 46,787 Austropuccinia psidii genes in OG0000017 (83.58% of the family; 34.92% of its representative proteome), requiring annotation/repeat/homology review before duplication interpretation. Recorded exact mappings and source hashes. No family was excluded and the live repair was not altered. Full resource planning, isolated staging and reconciliation remain pending.

### September 16: expanded ESMFold coupling completed with copy-review sensitivity

The revised ESMFold rate handoff completed optimization, audits, comparison and the 89-marker frame. Its first coupling attempt correctly stopped on one copy-review marker (301 sites). Reviewed isolated fitter/controller variants now require the exact flag universe and run both full89 exploratory inference and explicit omission88 sensitivity. Both completed all 24 fits, 48,000 bootstrap fits each and all 2,136/2,112 leave-one-out fits. Exact covariate-subset comparison and all 72 focal omission coefficients passed independent agreement checks. Sequence associations are positive across all specifications and their bootstrap intervals; interactions remain unresolved. These are conditional results, not confirmed orthology or causal structural change. Proactively superseded the waiting AlphaFold coupling controller with full124/omission123 reviewed handoffs, preserving its live rate producer and original scripts. Receipts, numerical summaries and failure/supersession provenance are archived; broader phylogeny, reconciliation and sensitivity work remains.
...
### 2026-09-16 — full protein accounting exposes unfinished clade discovery

Checked all 5,815,847 native proteins and the exact retained family partition. Independent native-reader readback confirms 1,330,409 outside-partition proteins (22.88%), all in the 462 added taxa; the 64 core taxa have complete representation in the saved partition. The intermediate discovery input contains these proteins plus exactly all 60,423 retained singletons. No proteins were lost during staging.

Installed-source review identifies the unfinished clade-specific discovery stage, which a direct --from-trees restart would skip. Updated README and orthology documentation: staged inputs support conditional retained-family reconciliation, not completion of full discovery. Both guide alternatives produce 45 discovery clades with different search dimensions. Discovery, singleton membership resolution, added gene trees, and guide/cross-clade sensitivity remain required before complete orthology can be claimed. The audit exports all omitted identities and per-taxon denominators without altering live inference. Evidence and reproducible readers are versioned under full_protein_family_coverage and unfinished_clade_discovery_scope.
...
### 2026-09-16 — complete-protein merge and source crosswalk prepared

Implemented the guide-specific merger that preserves 43,715 original multi-protein families and replaces all 60,423 singleton slots through discovery, enforcing exact single membership for all 5,815,847 input proteins. Contiguous new family indices have mandatory source-family and singleton-relocation crosswalks, avoiding accidental reuse of trees by old OG numbers. Two synthetic guide partitions and ten rejection cases passed; the full file-based command also passed against audited synthetic discovery outputs (99 proteins/25 families per guide), including a changed retained-tree index and singleton joining a larger family. The production plan is pinned but not launched or automatically queued: complete discovery, its readback and subsequent independent merged-file validation remain required. Live discovery workers continue advancing.
