...
### Fungal–outgroup root-split diagnostic

Both 526-tip homogeneous guides contain the exact bipartition separating all 501 designated fungal entries from the 25 designated outgroups. This identifies a compatible separating edge, not a supported final species-tree root or a location along that edge. No rooting or branch-length subdivision was applied to the saved trees.

The same diagnostic on the 70 completed marker trees finds an exact fungal–outgroup separation in 16 and no such split in 54. All 70 have at least two tips in each role, so no singleton-outgroup terminal edge is counted as informative agreement. Descendant-set results were independently checked by deleting edges from undirected graphs for all 72 trees, including separating-edge length accounting. All marker split-presence and support values match the previously audited marker split table.

The root partition is identical in the two guides, so their marker-support diagnostics agree exactly. At SH-aLRT cutoff 80, 14 marker trees show supported agreement, 50 show a supported incompatible split and six are unresolved. At cutoff 95, the corresponding counts are 9/36/25. These counts are from the incomplete, potentially completion-order-biased 70-marker snapshot; they are neither bootstrap proportions nor posterior confidence in a species-tree root. The maximum-conflict calculations inherit the earlier diagnostic's validation scope.

This disagreement must remain explicit in later reconciliation and ancestral analyses. It does not identify the biological cause: gene-tree estimation error, model misspecification, gene-copy issues, annotation problems and real discordance are alternatives requiring their planned checks. Do not select only the agreeing genes as a new baseline or impose gain/loss polarity on the provisional marker mappings. Repeat against all 125 completed marker trees and supported mixture/species-tree sensitivities before directional claims.

The script is `audit_fungal_outgroup_root_split.py`; diagnostic tables, source hashes and crosschecks are versioned under `metadata/fungal_outgroup_root_*`. Full output is `results/phylogeny/fungal-outgroup-root-diagnostic-v1`. Entry-level taxonomic uncertainty remains separate from this exact role-label partition check.
